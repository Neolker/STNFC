...
[#assign ExtiLine = ""]
[#assign IrqNumber = ""]
[#assign NFC8_INT_PIN_GPO_EXTI_LINE = ""]
[#assign NFC8_INT_PIN_GPO_EXTI_IRQn = ""]
[#assign NFC0XCOMMInstance1 = ""]
[#assign NFC08A1_GPO_PORT = ""]
[#assign NFC08A1_GPO_PIN = ""]
[#assign NFC06A1_LED1_PORT = ""]
[#assign NFC06A1_LED1_PIN = ""]
[#assign NFC06A1_LED2_PORT = ""]
[#assign NFC06A1_LED2_PIN = ""]
[#assign NFC06A1_LED3_PORT = ""]
[#assign NFC06A1_LED3_PIN = ""]
	...
	[#if SWIP.variables??]
		[#list SWIP.variables as variables]
			...
			[#if variables.value?contains("NFC06A1_8A1 BUS IO driver")]
				[#assign NFC0XCOMMInstance1 = IpInstance]
			[/#if]
			[#if variables.name?contains("BSP USART")]
				[#assign IrqNumber = variables.value]
			[/#if]
			[#if variables.name?contains("BSP BUTTON")]
				[#assign ExtiLine = variables.value]
			[/#if]
			[#if variables.value?contains("NFC8 GPO PIN")]
				[#assign NFC08A1_GPO_PORT = IpName]
				[#assign NFC08A1_GPO_PIN = IpInstance]
				[#assign NFC8_INT_PIN_GPO_EXTI_LINE = ExtiLine]
				[#assign NFC8_INT_PIN_GPO_EXTI_IRQn = IrqNumber]				
			[/#if]			
			[#if variables.value?contains("GREEN LED")]
				[#assign NFC06A1_LED1_PORT = IpName]
				[#assign NFC06A1_LED1_PIN = IpInstance]				
			[/#if]
			[#if variables.value?contains("BLUE LED")]
				[#assign NFC06A1_LED2_PORT = IpName]
				[#assign NFC06A1_LED2_PIN = IpInstance]				
			[/#if]
			[#if variables.value?contains("YELLOW LED")]
				[#assign NFC06A1_LED3_PORT = IpName]
				[#assign NFC06A1_LED3_PIN = IpInstance]				
			[/#if]			
		[/#list]
	[/#if]
  [/#list]
[/#if]
#define BMP_TEST ${NFC0XCOMMInstance1}
/* Includes ------------------------------------------------------------------*/
#include "logger.h"
#include "st_errno.h"
#include <string.h>
